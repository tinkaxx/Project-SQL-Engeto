...
K dispozici mám tabulky poskytující údaje o mzdách v různých odvětvích za určité období a také informace o cenách vybraných potravin (czechia_payroll, czechia_price). Dále jsou k dispozici různé číselníky, které mi pomohou rozklíčovat určité údaje v tabulkách (czechia_payroll_calculation, czechia_payroll_industry_branch, czechia_payroll_unit, czechia_payroll_value_type, czechia_price_category). K dispozici máme také informace o krajích a okresech ČR (czechia_region, czechia_district). A dále je možné při zpracování projektu využít informace o zemích na světě, i o jejich ekonomice (countries, economies).
